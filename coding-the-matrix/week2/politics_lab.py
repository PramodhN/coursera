...
def create_voting_dict(strlist):
    ...
    for line in strlist:
        ...
        d[splitline[0]] = [int(splitline[x]) for x in range(3,len(splitline))]
    return d
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    rec_a = voting_dict[sen_a]
    rec_b = voting_dict[sen_b]
    return sum([rec_a[x]*rec_b[x] for x in range(len(rec_a))])
...
def most_similar(sen, voting_dict):
    ...
    for k in voting_dict.keys():
        if k != sen:
            d[k] = policy_compare(sen, k, voting_dict)
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        -0.5
    """
    dotproducts = [policy_compare(sen, senb, voting_dict) for senb in sen_set]
    return sum(dotproducts)/len(dotproducts)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    votingrecords = [voting_dict[x] for x in voting_dict.keys() if x in sen_set]
    recordlength = len(votingrecords[0])
    averagelist = list()
    for x in range(recordlength):
        averagelist.append(0)
        for y in range(len(votingrecords)):
            averagelist[x] += votingrecords[y][x]
        averagelist[x] /= len(votingrecords)
    return averagelist
...
def bitter_rivals(voting_dict):
    ...
    Example: 
        ...
        True
    """
    similarity = len([voting_dict[x] for x in voting_dict.keys()][0])
    rivals = ('','')
    for i in voting_dict.keys():
        for j in voting_dict.keys():
            if i != j and similarity > policy_compare(i, j, voting_dict):
