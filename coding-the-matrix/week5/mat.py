...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    (dict1, dict2) = updatedDictionary(A.f,B.f,A.D)
    for x in A.D[0]:
        for y in A.D[1]:
...
def updatedDictionary(dict1,dict2,D):
    """
    :param dict1: a dictionary
    :param dict2: a dictionary
    :return: true if both have same values for all keys
    """
    for x in D[0]:
        for y in D[1]:
            if (x,y) not in dict1:
...
def add(A, B):
    ...
    for x in A.D[0]:
        for y in A.D[1]:
            resdict[(x,y)] = dict1[(x,y)] + dict2[(x,y)]
    return Mat(A.D, resdict)
...
def vector_matrix_mul(v, M):
    ...
    for C in M.D[1]:
      sum = 0
      for R in M.D[0]:
        if R in v.f and (R,C) in M.f:
          sum += (M.f[R, C] * v[R])
      mul[C] = sum
    return Vec(M.D[1], mul)
...
def matrix_vector_mul(M, v):
    ...
    for R in M.D[0]:
      sum = 0
      for C in M.D[1]:
        if C in v.f and (R,C) in M.f:
          sum += (M.f[R, C] * v[C])
      mul[R] = sum
    return Vec(M.D[0], mul)
    ...
    for x in A.D[0]:
        for y in B.D[1]:
            ...
            for z in B.D[0]:
                ...
                if (z,y) not in B.f:
                    B.f[(z,y)] = 0
                mul[(x,y)] += (A.f[(x,z)]*B.f[(z,y)])
    return Mat((A.D[0],B.D[1]), mul)
