...
def reflect_about(x1, y1, x2, y2):
    ...
    b = y2 - (math.tan(angle) * x2)
    return Mat(({'x','y','u'},{'x','y','u'}),{('x','x'):math.cos(2*angle), ('x','y'):math.sin(2*angle), ('x','u'):(-1*b*math.sin(2*angle)), ('y','x'):math.sin(2*angle), ('y','y'):(-1*math.cos(2*angle)), ('y','u'):(b + (b*math.cos(2*angle))), ('u','u'):1})
