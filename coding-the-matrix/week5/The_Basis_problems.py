...
def is_superfluous(S, v):
    ...
    if len(S) == 1:
        ...
        S_without_v = coldict2mat([vec for vec in S if vec != v])
        linear_combination = solve(S_without_v, v)
        return (v - S_without_v*linear_combination).is_almost_zero()
...
def is_independent(S):
    ...
    if len(S) == 0:
        ...
        superfluous = ([is_superfluous(S,v) for v in S])
        for val in superfluous:
            if val == True:
                return False
        return True
...
def exchange(S, A, z):
    ...
    for b in S:
        if b not in A:
            if is_superfluous(new_S, b) == True:
                return b
