...
def updatedDictionary(dict1,dict2,D):
    ...
    :return: updated u and v such that both have same set of keys
    """
    equalSet=set()
    for x in D:
        if x not in dict1:
...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    (dict1, dict2) = updatedDictionary(u.f,v.f,u.D)
    equalset = {dict1[k] == dict2[k] for k in u.D}
    if False in equalset:
        return False
    else:
        return True
