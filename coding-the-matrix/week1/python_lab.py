...
LofL_sum = sum([sum(s) for s in LofL])
...
L = ['A','B','C','D','E']
range_and_zip = list(zip(set(range(5)),L))
...
list_sum_zip = [sum(s) for s in tuple(zip([10, 25, 40],[1, 15, 20]))]
...
listdict2dict = { names[x]:y for (x,y) in id2salary.items() }
